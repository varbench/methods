...
//[e] MERGE BY TM
  ...
  if (NVMCCalMode == 1) {
    ...
    if (NCisAjsCktAlt > 0) {
      for (i = 0; i < NCisAjsCktAlt; i++) {
        fprintf(FileCisAjsCktAlt, "%d %d %d %d %d %d %d %d % .18e % .18e\n",
                CisAjsCktAltIdx[i][2], CisAjsCktAltIdx[i][3], CisAjsCktAltIdx[i][4], CisAjsCktAltIdx[i][5],
                CisAjsCktAltIdx[i][6], CisAjsCktAltIdx[i][7], CisAjsCktAltIdx[i][8], CisAjsCktAltIdx[i][9],
                creal(PhysCisAjsCktAlt[i]), cimag(PhysCisAjsCktAlt[i]));
      }
      fprintf(FileCisAjsCktAlt, "\n");
    }
    ...
    /* zvo_cisajscktaltcmuanucovapv.dat */
    if (NCisAjsCktAltCmuAnuCovApv > 0) {
      for (i = 0; i < NCisAjsCktAltCmuAnuCovApv; i++){
    fprintf(FileCisAjsCktAltCmuAnuCovApv, "% .18e % .18e\n",
    creal(PhysCisAjsCktAltCmuAnuCovApv[i]), cimag(PhysCisAjsCktAltCmuAnuCovApv[i]));
      }
      fprintf(FileCisAjsCktAltCmuAnuCovApv, "\n");
    }
