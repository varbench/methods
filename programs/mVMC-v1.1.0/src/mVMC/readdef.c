...
int CheckOctSite(const int iSite1, const int iSite2, const int iSite3, const int iSite4,
                 const int iSite5, const int iSite6, const int iSite7, const int iSite8, const int iMaxNum);
...
int GetInfoFourBodyGEx(FILE *fp, int **ArrayIdx, int Nsite, int NArray, char *defname);
...
//RBM
          ...
          case KWFourBodyGEx:
      	    cerr = ReadBuffInt(fp, &bufInt[IdxNFourBodyGEx]);
      	    break;
  ...
  NCisAjsCktAltCmuAnuCovApv = bufInt[IdxNFourBodyGEx];
  ...
  NPara = NProj + NSlater + NOptTrans + NProjBF + NRBM;
  ...
  NTotalDefInt = Nsite /* LocSpn */
                 ...
                 + 2 * NCisAjsCktAltCmuAnuCovApv /* CisAjsCktAltCmuAnuCovApv */
...
//RBM
        ...
        case KWOrbitalParallel:
          /*orbitalidxt.def------------------------------------*/
          fidx = NProj+NRBM + iNOrbitalAntiParallel;
        ...
        case KWFourBodyGEx:
   	      /*cisajscktaltcmuanucovapv.def--------------------------------*/
  	      if (GetInfoFourBodyGEx(fp, CisAjsCktAltCmuAnuCovApvIdx, Nsite, NCisAjsCktAltCmuAnuCovApv, defname) != 0)
  	       info = 1;
  	      break;
...
/**
 * @brief Check Site Number for a oct -> (siteA, siteB, siteC, siteD, siteE, siteF, siteG, siteH).
 * @param[in] iSite1 a site number on site A.
 * @param[in] iSite2 a site number on site B.
 * @param[in] iSite3 a site number on site C.
 * @param[in] iSite4 a site number on site D.
 * @param[in] iSite5 a site number on site E.
 * @param[in] iSite6 a site number on site F.
 * @param[in] iSite7 a site number on site G.
 * @param[in] iSite8 a site number on site H.
 * @param[in] iMaxNum Max site number
 * @author Rico Pohle (The University of Tokyo)
 **/
int CheckOctSite(
                 const int iSite1,
                 const int iSite2,
  ...
  if (CheckPairSite(iSite5, iSite6, iMaxNum) != 0) {
    return (-1);
  }
  if (CheckPairSite(iSite7, iSite8, iMaxNum) != 0) {
    return (-1);
  }
  return 0;
}
      ...
      switch (iKWidx) {
        case KWModPara:
          ...
          while (fgets(ctmp2, sizeof(ctmp2) / sizeof(char), fp) != NULL) {
            ...
            } else if (CheckWords(ctmp, "NSPar") == 0) {
               bufInt[IdxSPar] = (int) dtmp;
...
int GetInfoTwoBodyGEx(FILE *fp, int **ArrayIdx, int Nsite, int NArray, char *defname) {
  ...
  if (NArray == 0) return 0;
  while (fgets(ctmp2, sizeof(ctmp2) / sizeof(char), fp) != NULL) {
    sscanf(ctmp2, "%d %d %d %d %d %d %d %d %d %d\n", &x0, &x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9);
    ArrayIdx[idx][0] = x0;
    ArrayIdx[idx][1] = x1;
    ArrayIdx[idx][2] = x2;
    ArrayIdx[idx][3] = x3;
    ...
    ArrayIdx[idx][8] = x8;
    ArrayIdx[idx][9] = x9;
    if (CheckQuadSite(x2, x4, x6, x8, Nsite) != 0) {
      fprintf(stderr, "Error: Site index is incorrect. \n");
      info = 1;
      break;
    }
...
int GetInfoFourBodyGEx(FILE *fp, int **ArrayIdx, int Nsite, int NArray, char *defname) {
  ...
  if (NArray == 0) return 0;
  while (fgets(ctmp2, sizeof(ctmp2) / sizeof(char), fp) != NULL) {
    sscanf(ctmp2, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d \n",
    ...
    ArrayIdx[idx][16]  = x16;
    ArrayIdx[idx][17]  = x17;
    if (CheckOctSite(x2, x4, x6, x8, x10, x12, x14, x16, Nsite) != 0) {
      fprintf(stderr, "Error: Site index is incorrect. \n");
      info = 1;
      break;
    }
