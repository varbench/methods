...
//RBM
 ...
 /*[s] For BackFlow */
  ...
  CisAjsCktAltCmuAnuCovApvIdx = (int**)malloc(sizeof(int*)*NCisAjsCktAltCmuAnuCovApv);
  for(i=0;i<NCisAjsCktAltCmuAnuCovApv;i++) {
    CisAjsCktAltCmuAnuCovApvIdx[i] = pInt;
    pInt += 2;
  }
...
void FreeMemoryDef() {
  ...
  free(CisAjsCktAltCmuAnuCovApvIdx);
  free(CisAjsIdx);
  free(QPTransSgn);
  free(QPTrans);
  free(OrbitalIdx);
...
void SetMemory() {
  ...
  Proj     = Para;
  RBM      = Para + NProj;
  ProjBF   = Para + NProj + NRBM;
  Slater   = Para + NProj + NRBM + NProjBF;
  OptTrans = Para + NProj + NRBM + NProjBF + NSlater;
...
//for real
  ...
  if(NVMCCalMode==1){
    PhysCisAjs  = (double complex*)malloc(sizeof(double complex)
                    *(NCisAjs+NCisAjsCktAlt+NCisAjsCktAltDC+NCisAjsCktAltCmuAnuCovApv+NCisAjs+NCisAjsCktAlt));
    PhysCisAjsCktAlt   = PhysCisAjs       + NCisAjs;
    PhysCisAjsCktAltDC = PhysCisAjsCktAlt + NCisAjsCktAlt;
    PhysCisAjsCktAltCmuAnuCovApv = PhysCisAjsCktAltDC + NCisAjsCktAltDC;

    LocalCisAjs = PhysCisAjsCktAltCmuAnuCovApv + NCisAjsCktAltCmuAnuCovApv;
    LocalCisAjsCktAlt = LocalCisAjs + NCisAjs;
