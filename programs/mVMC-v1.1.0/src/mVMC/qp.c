...
void InitQPWeight() {
  ...
  // *********************************************
  // spin-parity projection: Rico Pohle 2020/08/19
  // *********************************************
    else if(NSPGaussLeg==2) {
    ...
    for(i=0;i<NSPGaussLeg;i++) {
      SPGLCos[i]    = cos(0.5 * beta[i])+0*I;
      SPGLSin[i]    = sin(0.5 * beta[i])+0*I;
      SPGLCosSin[i] = SPGLCos[i]*SPGLSin[i]+0*I;
      SPGLCosCos[i] = SPGLCos[i]*SPGLCos[i]+0*I;
      SPGLSinSin[i] = SPGLSin[i]*SPGLSin[i]+0*I;
	  ...
	  printf("spin-parity projection for NSPGaussLeg = 2 uses NSPar = 0 (even) and NSPar = 1 (odd) wave functions");
