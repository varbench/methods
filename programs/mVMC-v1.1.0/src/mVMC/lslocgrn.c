...
double complex calHCA1(const int ri, const int rj, const int s,
  ...
  RequestWorkSpaceComplex(NQPFull*(Nsize*Nsize+1) + NRBM_PhysLayerIdx+Nneuron);
...
double complex calHCA2(const int ri, const int rj, const int s,
  ...
  RequestWorkSpaceComplex(NQPFull + NRBM_PhysLayerIdx+Nneuron); /* for GreenFunc1 */
  RequestWorkSpaceThreadInt(Nsize+Nsite2+NProj+6);
  RequestWorkSpaceThreadComplex(NQPFull+3*Nsize + NRBM_PhysLayerIdx+Nneuron);
...
#pragma omp parallel default(shared)\
  private(myEleIdx,myEleNum,myBufferInt,myBuffer,myValue,myRsi,myRsj,myRBMCntNew)  \
  reduction(+:v)
  {
    ...
    for(idx=0;idx<NTransfer;idx++) {
      ...
      myValue -= ParaTransfer[idx]
        * GreenFunc2(rk,rl,ri,rj,sk,s,ip,myEleIdx,eleCfg,myEleNum,eleProjCnt,myBufferInt,rbmCnt,rbmCntNew,myBuffer);
        //* GreenFunc2RBM(rk,rl,ri,rj,sk,s,ip,myEleIdx,eleCfg,myEleNum,eleProjCnt,myBufferInt,rbmCnt,rbmCntNew,myBuffer);
      /* Caution: negative sign */
    }
