...
def run_2DTFIM(numsteps = 2*10**4, systemsize_x = 5, systemsize_y = 5, Bx = +2, num_units = 50, numsamples = 500, learningrate = 5e-3, seed = 111):
    ...
    for u in units:
        ending+='_{0}'.format(u)
    filename='./Check_Points/RNNwavefunction_2DVanillaRNN_'+str(Nx)+'x'+ str(Ny) +'_Bx'+str(Bx)+'_lradap'+str(lr)+'_samp'+str(numsamples)+ending+'.ckpt'
    savename = '_2DTFIM'
    ...
    #         meanEnergy=np.load('../Check_Points/meanEnergy_2DVanillaRNN_'+str(Nx)+'x'+ str(Ny) +'_Bx'+str(Bx)+'_lradap'+str(lr)+'_samp'+str(numsamples)+ending  + savename +'.npy').tolist()
    #         varEnergy=np.load('../Check_Points/varEnergy_2DVanillaRNN_'+str(Nx)+'x'+ str(Ny) +'_Bx'+str(Bx)+'_lradap'+str(lr)+'_samp'+str(numsamples)+ending  + savename +'.npy').tolist()
    #-----------
...
#                 print("sampling ended: "+ str(end - start))
            ...
            #Comment if you dont want to save or if saving is not working
                if it%10==0:
                  ...
                  np.save('./Check_Points/meanEnergy_2DVanillaRNN_'+str(Nx)+'x'+ str(Ny) +'_Bx'+str(Bx)+'_lradap'+str(lr)+'_samp'+str(numsamples)+ending  + savename +'.npy', meanEnergy)
                  np.save('./Check_Points/varEnergy_2DVanillaRNN_'+str(Nx)+'x'+ str(Ny) +'_Bx'+str(Bx)+'_lradap'+str(lr)+'_samp'+str(numsamples)+ending + savename +'.npy', varEnergy)
            ...
            print("End of training! Now saving model parameters and estimating the RNN's energy")

            print("Saving the model parameters:")
            saver.save(sess,path+'/'+filename)

            numsamples_estimation = 2*(10**6)
            numbatches = 10**4
    ...
    return meanEnergy, varEnergy, meanE_final, errE_final
