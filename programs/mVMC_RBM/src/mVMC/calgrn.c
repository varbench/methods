...
void CalculateGreenFunc(const double w, const double complex ip, int *eleIdx, int *eleCfg,
  ...
  RequestWorkSpaceComplex(NQPFull+2*Nsize + NRBM_PhysLayerIdx+Nneuron);
  ...
  private(myRBMCntNew, myEleIdx,myEleNum,myProjCntNew,myBuffer,idx)
  {
    ...
    // calculate 4 BodyGreenFunction in indirect way from 2 BodyGreenfunctions
    #pragma omp for private(idx,idx0,idx1) nowait
    for(idx=0;idx<NCisAjsCktAltCmuAnuCovApv;idx++) {
      idx0 = CisAjsCktAltCmuAnuCovApvIdx[idx][0];
      idx1 = CisAjsCktAltCmuAnuCovApvIdx[idx][1];
      PhysCisAjsCktAltCmuAnuCovApv[idx] += w * LocalCisAjsCktAlt[idx0]*conj(LocalCisAjsCktAlt[idx1]);
    }
