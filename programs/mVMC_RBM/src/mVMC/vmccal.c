...
#pragma loop noalias
      ...
      if(FlagOptTrans>0) { // this part will be not used
        calculateOptTransDiff(SROptO+2*NProj+2*NRBM+2*NSlater+2, ip); //TBC
      }
...
// only for real variables
    ...
    /* CisAjs, CisAjsCktAlt, CisAjsCktAltDC, CisAjsCktAltCmuAnuCovApv*/
    n = NCisAjs+NCisAjsCktAlt+NCisAjsCktAltDC+NCisAjsCktAltCmuAnuCovApv;
    vec = PhysCisAjs;
#pragma omp parallel for default(shared) private(i)
    for(i=0;i<n;i++) vec[i] = 0.0+0.0*I;
