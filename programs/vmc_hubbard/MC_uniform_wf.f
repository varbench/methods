      ...
      implicit none 
      include 'mpif.h'
      integer*4 nx,idim,ivc,indt,izeth,L,nelm,npm,nelm2
     ...
     >,nraw,iflag,ind,nwtot,Lsq,irstart
      parameter(npm=300)
     ...
     >,ratio,ener,ener2,diagsum,wtot,rata,time,ran
     >,sflip,p1,p2,costz,tacc,mywbra,myener,myener2,mydiagsum,mywtot
      integer*2,allocatable :: ivic(:,:),ivicr(:,:),icord(:,:)
     >,iconf(:,:),jbraw(:),kelup(:,:),keldo(:,:),irif(:),multirif(:)
     >,ivicns(:,:,:),ivicn(:,:,:),ivicnr(:,:,:),ivicnb(:,:,:)
     >,kel(:),iconfnew(:)
      integer*4,allocatable :: indtn(:),indtns(:),indtnr(:),indtnb(:)
     >,jbra(:),iconfm(:),ipsip(:),ipos(:)
      real*4,allocatable :: wconfw(:)
      real*8,allocatable :: v(:,:),vSz(:,:),vhd(:,:),t(:),alphav(:)
     >,hopupdo(:),z(:,:),vjz(:),vj(:),vjh(:),ddw(:),dsw(:)
     >,dek(:),dup(:),par(:),matrix(:,:),eig(:)
     >,opmat(:,:,:),back(:,:),alpha(:)
     >,gh(:),ghSz(:),ghhd(:),thop(:,:),thopv(:,:)
     >,jhop(:,:),jhopv(:,:),sfhopv(:,:)
     >,wconfn(:),econf(:)
     >,zeta(:),tabpip(:,:),tabpipSz(:,:),tabpiphd(:,:)
     >,table(:,:,:),diag(:),diagfn(:),wsto(:)
     >,enert(:),enert2(:),diagpart(:)
     >,sovdt(:),sov(:,:,:),mysov(:,:,:),myetotw(:),etotw(:)
     >,zet(:,:),cdet(:,:),matr(:,:)
     >,inver(:,:,:),amat(:,:,:),scalpar(:)
     >,psip(:),work(:),psim(:,:),zetscra(:)
      logical,allocatable :: iocc(:)
      character*11 filename,chara
      integer *4 ierr,size,rank,sizer

       namelist /sizepar/ nx,L,idim,ivc
       namelist /montecarlo/ itestr,iseed,iopt,iread,nwtot,ngen
     >,nscra,epst
       namelist /fixednode/ etry,tbra,gamma
       namelist /vmc/ nbra,sflip
       namelist /optimization/ nbra,sflip,nweight,ibinit,epsdgel,tau
     >,nraw

      call mpi_init(ierr)
      if (ierr.ne.mpi_success) print*,'some problem in parallelization!'
      call mpi_comm_size(MPI_COMM_WORLD,size,ierr)
      call mpi_comm_rank(MPI_COMM_WORLD,rank,ierr)
      print*, ' my rank is', rank

      write(chara,'("randseed",I0)') rank
            chara=trim(chara)
      ...
      call mpi_bcast(itestr,1,mpi_integer,0,mpi_comm_world,ierr)
      ...
      call mpi_bcast(nwtot,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(ngen,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(nscra,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(epst,1,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(nx,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(L,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(idim,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(ivc,1,mpi_integer,0,mpi_comm_world,ierr)


      if(nx.eq.0) then
       if(rank.eq.0) write(6,*) 'nx must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(L.eq.0) then
       if(rank.eq.0) write(6,*) 'L must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(idim.eq.0) then
       if(rank.eq.0) write(6,*) 'idim must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(ivc.eq.0) then
       if(rank.eq.0) write(6,*) 'ivc must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(itestr.eq.0) then
       if(rank.eq.0) write(6,*) 'itestr must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(iseed.eq.0) then
       if(rank.eq.0) write(6,*) 'iseed must be specified'
       call mpi_finalize(ierr)
       stop
      endif
      if(ngen.eq.0) then
       if(rank.eq.0) write(6,*) 'ngen must be specified'
       call mpi_finalize(ierr)
       stop
      endif     
      ...
      call mpi_bcast(gamma,1,mpi_double_precision,0,mpi_comm_world,ierr)
       ...
       if(tbra.eq.0.d0) then
        if(rank.eq.0) write(6,*) 'tbra must be specified'
        call mpi_finalize(ierr)
        stop
       endif
      ...
      call mpi_bcast(sflip,1,mpi_double_precision,0,mpi_comm_world,ierr)
      ...
      call mpi_bcast(epsdgel,1,mpi_double_precision,0,mpi_comm
     >_world,ierr)
      call mpi_bcast(tau,1,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(nraw,1,mpi_integer,0,mpi_comm_world,ierr)

       if(nweight.eq.0) then
        if (rank.eq.0) write(6,*) 'nweight must be specified'
        call mpi_finalize(ierr)
        stop
       endif
       if(ibinit.eq.0) then
        if(rank.eq.0) write(6,*) 'ibinit must be specified'
        call mpi_finalize(ierr)
        stop
       endif
       ...
       if(rank.eq.0) write(6,*) 'Wrong itestr',itestr
       call mpi_finalize(ierr)
       stop
      endif


      allocate(scalpar(npm),ipos(npm))      
      ...
      call mpi_bcast(scalpar,npm,mpi_double_precision,0
     >,mpi_comm_world,ierr)

      if(mod(nwtot,size).ne.0) then
       if(rank.eq.0) write(6,*)'Walker number has
     > to be a multiple of processors'
       call mpi_finalize(ierr)
       stop
      endif

c nw=number of walkers per process
      nw=nwtot/size
      indt=2*ivc*idim
      izeth=ivc*idim
      npsov=npm*npm
      ...
      Lz=L*indt
      Lsq=L**2
      iscramax=8*L*L+22*L+npsov+13*npm


      allocate(ivic(L,indt),ivicr(L,indt),icord(L,idim),iconf(L,nw)
     >,jbraw(nw),kelup(L,nw),keldo(L,nw),irif(L),multirif(L)
     >,ivicns(L,8,L),ivicn(L,8,L),ivicnr(L,8,L),ivicnb(L,8,L)
     >,kel(L),iconfnew(L)
     >,indtn(L),indtns(L),indtnr(L),indtnb(L),jbra(nw)
     >,iconfm((nw*L)/16+1),ipsip(L+2*nw+2*npm)
     >,wconfw(nw)
     >,v(L,L),vSz(L,L),vhd(L,L),t(izeth),alphav(izeth)
     ...
     >,wconfn(nw),econf(Nw*npm)
     >,zeta(nw+1),tabpip(L,nw),tabpipSz(L,nw),tabpiphd(2*L,nw)
     >,table(L,indt,nw),diag(nw),diagfn(nw),wsto(nw)
     >,enert(nw),enert2(nw),diagpart(nw)
     >,sovdt(npsov),sov(npm,npm,2),mysov(npm,npm,2),etotw(npm)
     >,myetotw(npm),zet(nelm,L),cdet(L,L),matr(L,L)
     >,inver(nelm,nelm,nw),amat(L,L,2)
...
c iopt=2   begin with old conf but restart averages  
     ...
     >,iessw,ieskin,iesup,muf,ioptir,alphav,par,eig

      endif


       write(filename,'("fort.11.",I0)') rank
            filename=trim(filename)
       open(unit=11,file=filename,form='unformatted'
     >,status='unknown')
       write(filename,'("fort.15.",I0)') rank
            filename=trim(filename)
       if(iread.eq.1) open(unit=15,file=filename
     >,form='unformatted',status='unknown')
       if(iread.eq.1) rewind(15)


      call mpi_bcast(v,Lsq,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(vsZ,Lsq,mpi_double_precision,0
     >,mpi_comm_world,ierr)
      call mpi_bcast(vhd,Lsq,mpi_double_precision,0
     >,mpi_comm_world,ierr)
      call mpi_bcast(t,izeth,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(U,1,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(ivicr,Lz,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(icord,L*idim,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(irif,L,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(nelup,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(neldo,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(z,L2*L2,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(ivicn,8*Lsq,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(indtn,L,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(ivicns,8*Lsq,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(indtns,L,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(ivicnr,8*Lsq,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(indtnr,L,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(ivicnb,8*Lsq,mpi_integer2,0,mpi_comm_world,ierr)
      call mpi_bcast(indtnb,L,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(vjz,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(vj,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(vjh,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(back,4*L,mpi_double_precision,0
     >,mpi_comm_world,ierr)
      call mpi_bcast(ddw,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(dsw,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(dek,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(dup,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(iesm,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(iesd,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(iesh,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(iback,4,mpi_integer,0,mpi_comm_world,ierr)
      ...
      call mpi_bcast(muf,1,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(ioptir,1,mpi_integer,0,mpi_comm_world,ierr)
      call mpi_bcast(alphav,izeth,mpi_double_precision,0
     >,mpi_comm_world,ierr)
      call mpi_bcast(par,L,mpi_double_precision,0,mpi_comm_world,ierr)
      call mpi_bcast(eig,L2,mpi_double_precision,0,mpi_comm_world,ierr)
      ...
      if(itestr.eq.-2) then
       if(nmat.gt.npm) then
        if(rank.eq.0)  write(6,*) 'Increase npm',nmat,npm
        call mpi_finalize(ierr)
        stop
       endif
      endif
     ...
     > call makesovop(L,nelup,neldo,indtn,ivicn,indtns
     >,ivicns,indtnr,ivicnr,iesfree,iessw,iesup,ieskin,matrix,par
     >,ioptir,z,eig,psip,opmat)
     ...
     >,wconfn,z,ipsip,psip,par,epst,ioptir,rank)
     ...
     >,nelupr,neldor
       ...
       if(Lr.ne.L) then
        if(rank.eq.0) write(6,*) 'The calculation 
     >must continue with L=',Lr
        call mpi_finalize(ierr) 
        stop
       endif 
       if(npr.ne.np) then
        if(rank.eq.0) write(6,*) 'The calculation 
     >must continue with np=',npr
        call mpi_finalize(ierr) 
        stop
       endif 
       if(nwr.ne.nw) then
        if(rank.eq.0) write(6,*) 'The calculation 
     >must continue with nw=',nwr
        call mpi_finalize(ierr) 
        stop
       endif
       if(nelupr.ne.nelup) then
        if(rank.eq.0) write(6,*) 'The calculation 
     >must continue with nelup=',nelupr
        call mpi_finalize(ierr) 
        stop
       endif
       if(neldor.ne.neldo) then
        if(rank.eq.0) write(6,*) 'The calculation 
     >must continue with neldo=',neldor
        call mpi_finalize(ierr) 
        stop
       endif
      ...
      if(rank.eq.0) write(6,*) 'Starting ngg, ngn=',ngg,ngn
...
c======================================================================
     ...
     >,back,iback,zetscra,hopupdo,rank,ierr)
     ...
     >,ivicnb,indtnb,iback,back,rank,ierr)
...
c --------------DO sui walkers--------------------------
     ...
     >,kel,ivicnb,indtnb,iback,back,rank,ierr)
          zeta(1)=1.d0-drand1()
     ...
     1      (itest.eq.2.and.ntry.eq.1)) then
          ...
          if(itest.eq.1) then
           if(ttry.ne.tleft) then 
            if(rank.eq.0) write(6,*) ' error impossible condition
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     >,epst,cdet,kel,cont,psip,work,ipsip,rank,ierr)
           endif        
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c Calculation of observables
     ...
     >,kelup(1,j),keldo(1,j),econf(j+nwfree),ioptir,iback)
       ...
       myener=enert(1)*wconfn(1)
       myener2=enert2(1)*wconfn(1)
       mydiagsum=diagpart(1)*wconfn(1)
       mywtot=wconfn(1)
       do j=2,nw
        mywtot=mywtot+wconfn(j)
        myener=myener+enert(j)*wconfn(j)
        myener2=myener2+enert2(j)*wconfn(j)
        mydiagsum=mydiagsum+diagpart(j)*wconfn(j)
       enddo
       call mpi_reduce(myener,ener,1,mpi_double_precision,mpi_sum,0
     >,mpi_comm_world,ierr)
       call mpi_reduce(mywtot,wtot,1,mpi_double_precision,mpi_sum,0
     >,mpi_comm_world,ierr)
       call mpi_reduce(myener2,ener2,1,mpi_double_precision,mpi_sum,0
     >,mpi_comm_world,ierr)
       call mpi_reduce(mydiagsum,diagsum,1,mpi_double_precision
...
c---------------------------------------------------------------------
       ...
       if(itest.eq.1) then 
        ...
        zeta(nw+1)=drand1()
        call branchingo(Nw,wconfn,mywbra,zeta,icdiff,ipsip,jbra)
        sumdiff=sumdiff+icdiff

        call mpi_reduce(mywbra,wbra,1,mpi_double_precision,mpi_sum,0
     >,mpi_comm_world,ierr)
       ...
       elseif(itestr.eq.-2) then 
        ...
        IF(i.eq.inext) THEN 
         ...
         call reweight0(Nw,np,npm,etotw,myetotw,ipsip,psip,alpha
     >,sov,mysov,sovdt,econf,epsdgel,1,L,enert,scalpar,rank,ierr)
     ...
     >,vsz,v,vhd,z,muf,ivicr,indt,alphav,ioptir,eig)
     ...
     >,back,iback,zetscra,hopupdo,rank,ierr)
         ...
         call mpi_bcast(ngn,1,mpi_integer,0,mpi_comm_world,ierr)
        ...
        ELSE  ! i.eq.init
         ...
         call reweight0(Nw,np,npm,etotw,myetotw,ipsip,psip,alpha
     >,sov,mysov,sovdt,econf,epsdgel,0,L,enert,scalpar,rank,ierr)
     ...
     >,wsto,diagfn,diag,kelup,keldo,enert)
       ...
       if(iread.eq.1) then 
        ngg=ngg+1
        if(rank.eq.0) write(12) wbra,wtot,ener,ener2,diagsum
        write(15) mywbra,(iconfm(k),k=1,nint)
     ...
     >,nelup,neldo
      ...
      write(6,*) ' Total time spent by processor #',rank,'is',time
      time=time-timescra
      ...
      deallocate(ivic,ivicr,icord,iconf,jbraw,kelup,keldo,irif,multirif
     >,ivicns,ivicn,ivicnr,ivicnb,kel,iconfnew,indtn,indtns,indtnr
     >,indtnb,jbra,iconfm,ipsip,ipos, wconfw,v,vSz,vhd,t,alphav
     >,hopupdo,z,vjz,vj,vjh,ddw,dsw,dek,dup,par,matrix,eig,opmat,back,
     >alpha,gh,ghSz,ghhd,thop,thopv,jhop,jhopv,sfhopv,wconfn,econf
     >,zeta,tabpip,tabpipSz,tabpiphd,table,diag,diagfn,wsto
     >,enert,enert2,diagpart,sovdt,sov,mysov,etotw,myetotw,zet,cdet
     >,matr,inver,amat,scalpar,psip,work,psim,zetscra,iocc)  
...
c       write(6,*) 'refused singular =',cdet
      ...
      if(iref.eq.0) then
       write(6,*) 'all the configurations accepted in processor', rank
      else
       write(6,*) '% refused configurations in processor', rank, '='
     >,dble(iref)/dble(igen)
      endif
     ...
     >,back,iback,zet,hopupdo,rank,ierr)
      implicit none
      include 'mpif.h'
      integer*4 Nw,L,nelup,neldo,nel,nelm,i,j,k,h,jn,info,iopt
      integer*4 ipsip(L),indtnb(*),iback(4),rank,ierr
     ...
     >,matr,iopt,iocc,ivicnb,indtnb,iback,back,rank,ierr)
      implicit none
      include 'mpif.h'
      integer*4 L,idim,ivc,izeta,i,j,jn,ivcd,iopt,kj
     >,nelm,nel,cont,k,h,info,rank,ierr
      integer*2 ivic(L,*),ivicnb(L,8,*),iconf(L),kelup(L),keldo(L)
     >,kel(*),iconfnew(L)
      integer*4 ipsip(*),indtnb(*),iback(4)
...
c====================================================================
      ...
      subroutine reweight0(Nw,np,npm,etotw,myetotw,ipip,psip,alpha
     >,sov,mysov,sovdt,econf,epst,iweight,L,enert,scalpar,rank,ierr)
      implicit none 
      include 'mpif.h'
      integer*4 Nw,np,npp,i,j,k,info,irank,npm,iweight,ind,L
      integer*4 ipip(*),sovsize,rank,ierr
      real*8 dble,cost,epst
      real*8 psip(*),alpha(*),sov(npm,npm,*),sovdt(*),econf(nw,*)
     >,etotw(*),enert(*),scalpar(*),mysov(npm,npm,*),myetotw(*)
      ...
      IF(np.ne.0) THEN 
       ...
       do i=1,npp
        do j=i,npp
         mysov(i,j,1)=econf(1,i)*econf(1,j)*cost
        enddo
       enddo
       do i=2,nw
        call dsyr('U',npp,cost,econf(i,1),nw,mysov,npm) 
       enddo

       do i=1,npp
        do j=i+1,npp
         mysov(j,i,1)=mysov(i,j,1)
        enddo
       enddo
       ...
       do j=1,npp
        do i=1,npp
         ind=ind+1
         sovdt(ind)=sovdt(ind)+mysov(i,j,1)
         mysov(i,j,2)=sovdt(ind)
        enddo
       enddo

       do j=1,npp
        do i=1,nw
         myetotw(j)=myetotw(j)-cost*L*enert(i)*econf(i,j)
        enddo
       enddo

       if(iweight.eq.1) then 

        call mpi_reduce(myetotw,etotw,npm,mpi_double_precision,mpi_sum,0
     >,mpi_comm_world,ierr)
        call mpi_reduce(mysov,sov,sovsize
     ...
     >,epst,irank,ipip,info,0,scalpar)
        ...
        call mpi_bcast(psip,npp,mpi_double_precision,0
     >,mpi_comm_world,ierr)
        call mpi_bcast(info,1,mpi_integer,0
     >,mpi_comm_world,ierr)

        if(info.ne.0) then 
         if(rank.eq.0) write(6,*) 'Info=',info
         call mpi_finalize(ierr)
         stop
        else
         do i=1,npp
...
c Restore matrix sov for the calculation of the norm
      ...
      subroutine makesovop(nh,nelup,neldo,indtn,ivicn,indtns
     >,ivicns,indtnr,ivicnr,iesfree,iessw,iesup,ieskin,matrix,ss
     >,iopti,z,eig,psip,sov)
      implicit none 
      integer*4 nh,nel,nelup,neldo,k,i,j
      integer*4 ieskin,iesfree,iessw,iesup,ndim,np
     ...
     1,sov(2*nh,2*nh,*),eig(*)
      ...
      do k=1,np
       ...
       if(k.le.iesfree) then 
        call upmatrix(nh,nelup,neldo,indtn,ivicn,indtns
     1,ivicns,indtnr,ivicnr,k,0,0,0,matrix,ss,iopti)

       elseif(k.le.iesfree+iessw) then 
        call upmatrix(nh,nelup,neldo,indtn,ivicn,indtns
     1,ivicns,indtnr,ivicnr,0,k-iesfree,0,0,matrix,ss,iopti)

       elseif(k.le.iesfree+iessw+iesup) then 
        call upmatrix(nh,nelup,neldo,indtn,ivicn,indtns
     1,ivicns,indtnr,ivicnr,0,0,k-iesfree-iessw,0,matrix,ss,iopti)

       elseif(k.ge.iesfree+iessw+iesup+1) then 
        call upmatrix(nh,nelup,neldo,indtn,ivicn,indtns
     1,ivicns,indtnr,ivicnr,0,0,0,k-iesfree-iessw-iesup,matrix,ss,iopti)
     ...
     1,sov(1,1,k),ndim)
      ...
      subroutine upmatrix(nh,nelup,neldo,indtn,ivicn,indtns
     >,ivicns,indtnr,ivicnr,iesfree,iessw,iesup,ieskin,matrix,ss,iopti)
      implicit none 
      integer*4 nh,iq,ip,nel,nelup,neldo,k,jn
      integer*4 iesfree,iessw,iesup,ieskin,iopti
      integer*4 indtn(*),indtns(*),indtnr(*)
      integer*2 ivicn(nh,8,*),ivicns(nh,8,*),ivicnr(nh,8,*)
      real*8 ss(nh),matrix(2*nh,2*nh)
...
c     cycle over back3  
     ...
     >,vsz,v,vhd,z,muf,ivic,izeta,t,iopti,eig)
      implicit none 
...
c Determinant
     ...
     >,cont,psip,work,ipsip,rank,ierr)
      implicit none
      include 'mpif.h'
      integer*4 L,nel,cont,i,j,info,k,h1,h2,nelm,ind1,ind2,rank,ierr
      integer*2 kel(*)
      integer*4 ipsip(*)
     ...
     >,psip(2*nelm,2*cont),work(*),ginv
      ...
      if(info.ge.0) then
       ...
       if(rank.eq.0) write(6,*)'Problem in ratiovar'
       call mpi_finalize(ierr)
       stop
      endif
      ...
      if(info.ne.0)then
       if(rank.eq.0) write(6,*)'some problem in dgetri'
       call mpi_finalize(ierr)
       stop
      else
       ginv=-1.d0/g
...
c from scratch
     ...
     >,cont,h,k,i,jn,izeta,info,jj,rank,ierr
      integer*2 ivic(L,*),ivicnb(L,8,*),iconf(L),kelup(L),keldo(L)
     >,kel(*),iconfnew(L)
      integer*4 ipsip(*),indtnb(*),iback(4)
     ...
     >,psip,ipsip,cont,cdet,matr,kel,ivicnb,indtnb,iback,back,rank,ierr)
      implicit none
      include 'mpif.h'
      integer*4 L,i,j,jn,indspin,iopt,nelm,izeta
     >,cont,nel,k,h,info,rank,ierr
