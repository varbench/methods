...
c---------------------------------------------------------------------
     ...
     >,jj,kmax,iflagdxy,iflag,icount,stripe
     >,iesm,iesd,iesh,iesfree,iessw,iesup,ieskin,ibc,kl
     >,iback(4)
      parameter(nx=48,nh=288,idim=2,ivc=2,indt=ivc*idim,izeta=2*indt
     >,nind=nh)
...
c the last bonds have negative sign with respect to s-wave
     ...
     >,iesup,ieskin,vjz,vj,vjh,dd,dsw,muf,stripe,dup,dek,w,eigenval,psip
...
999   format(i5,3x,3(f14.10,2x))
     ...
     >,iessw,ieskin,iesup,muf,stripe,ioptr,alphav,par,eigenval
...
c *** table of idiff
     ...
     >,i,k,j,jn,ind,iopt,ibc,n1,n2,n3,stripe
     ...
     >,ss(nh),psip(*),vj(*),vjz(*),vjh(*),dek1,pi
     >,muf,matrix(2*nh,2*nh),eigenval(2*nh),t(*)
     >,vsz(nh,nh),v(nh,nh),vhd(nh,nh),dstripe
