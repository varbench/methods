...
c *** Lattice
      ...
      REAL(8), dimension(:), allocatable :: qxv,qyv
...
c     irange is the number of couplings in the Hamiltonian
      ...
      icheck=0
      do i=1,ndim
       if(dabs(Qx-qxv(i)).lt.1.d-7.and.dabs(Qy-qyv(i)).lt.1.d-7)
     >  icheck=icheck+1
      enddo
      if(icheck.ne.1) then
      ...
      do i=1,ndim
       do j=1,nbase
        icount=icount+1
        argo=2.d0*pi*(x(i)*Qx+y(i)*Qy+phi(j))
...
999   format(i5,3x,3(f14.10,2x))
     ...
     >,donsite_r,donsite_i,dAF,alphai
      ...
      DEALLOCATE(x,y,qxv,qyv,phi,alphai)
