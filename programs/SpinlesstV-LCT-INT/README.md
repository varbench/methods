...
Copied from https://github.com/wangleiphy/SpinlesstV-LCT-INT . This code uses $V \sum_{\langle i, j \rangle} (n_i - \frac{1}{2}) (n_j - \frac{1}{2})$, so the energy is lower by $\frac{1}{4} d N V$ than the result in the VarBench dataset, where $d$ is the lattice dimension.
